...
- `main` - Points to most up-to-date code and depends on the latest release of parse-server. This version of parse-hipaa is **built with [parse-hipaa-dashboard](https://github.com/netreconlab/parse-hipaa-dashboard)**. This tag can contain breaking changes
- `x.x.x` - Points to a specific released version. This version of parse-hipaa is **built with [parse-hipaa-dashboard](https://github.com/netreconlab/parse-hipaa-dashboard)**
- `x.x.x-slim` - Points to a specific released version. This is smallest possible image of `parse-hipaa` and it **does not contain [parse-hipaa-dashboard](https://github.com/netreconlab/parse-hipaa-dashboard)**
- `x.x.x-alpha/beta` - Points to most up-to-date code and depends on alha/beta releases of parse-server. This version of parse-hipaa is **built with [parse-hipaa-dashboard](https://github.com/netreconlab/parse-hipaa-dashboard)**. This tag can contain breaking changes

### Recommendations
Any/all of the tagged servers can be used in combination with eachother to build a [High Availability](https://en.wikipedia.org/wiki/High-availability_cluster)(HA) server-side application. For example, your HA cluster may consist of: (1) [nginx](https://www.nginx.com/resources/glossary/nginx/) reverse proxy/load balancer, (1) `x.x.x` `parse-hipaa` server, and (2) `x.x.x-slim` `parse-hipaa` servers , (1) [Percona Monitor and Management}(https://www.percona.com/software/database-tools/percona-monitoring-and-management) server

- `latest` or `x.x.x` - Use one of these images if you only plan to have one stand-alone `parse-hipaa` server or you want one of your servers to also provide [parse-hipaa-dashboard](https://github.com/netreconlab/parse-hipaa-dashboard) ability
